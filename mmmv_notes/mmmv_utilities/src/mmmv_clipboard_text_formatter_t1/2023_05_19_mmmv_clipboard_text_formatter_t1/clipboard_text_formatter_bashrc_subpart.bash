...
#     ruby, bash, par, (sed|gsed), nice, echo, printf, rm, tr, wc, 
#     ( xclip | ( clip.exe,powershell.exe ) )
# 
#==========================================================================

# Call example: mmmv_cat_par_sed_t1 ./x.txt
alias mmmv_cat_par_sed_30_t1="S_CMD_GNU_SED=\"sed\" ; if [ \"\`uname -a | grep -i 'BSD' \`\" != '' ]; then S_CMD_GNU_SED=\"gsed\" ; fi ; wait ; mmmv_polish_ABC_2_A_C_B_exec_t1 'cat ' \" | \$S_CMD_GNU_SED -e 's/[[:blank:]]\+/ /g' | par 30 | \$S_CMD_GNU_SED -e  's/^/    /g'\" " 
alias mmmv_cat_par_sed_40_t1="S_CMD_GNU_SED=\"sed\" ; if [ \"\`uname -a | grep -i 'BSD' \`\" != '' ]; then S_CMD_GNU_SED=\"gsed\" ; fi ; wait ; mmmv_polish_ABC_2_A_C_B_exec_t1 'cat ' \" | \$S_CMD_GNU_SED -e 's/[[:blank:]]\+/ /g' | par 40 | \$S_CMD_GNU_SED -e  's/^/    /g'\" " 
alias mmmv_cat_par_sed_50_t1="S_CMD_GNU_SED=\"sed\" ; if [ \"\`uname -a | grep -i 'BSD' \`\" != '' ]; then S_CMD_GNU_SED=\"gsed\" ; fi ; wait ; mmmv_polish_ABC_2_A_C_B_exec_t1 'cat ' \" | \$S_CMD_GNU_SED -e 's/[[:blank:]]\+/ /g' | par 50 | \$S_CMD_GNU_SED -e  's/^/    /g'\" "
alias mmmv_cat_par_sed_60_t1="S_CMD_GNU_SED=\"sed\" ; if [ \"\`uname -a | grep -i 'BSD' \`\" != '' ]; then S_CMD_GNU_SED=\"gsed\" ; fi ; wait ; mmmv_polish_ABC_2_A_C_B_exec_t1 'cat ' \" | \$S_CMD_GNU_SED -e 's/[[:blank:]]\+/ /g' | par 60 | \$S_CMD_GNU_SED -e  's/^/    /g'\" "
alias mmmv_cat_par_sed_70_t1="S_CMD_GNU_SED=\"sed\" ; if [ \"\`uname -a | grep -i 'BSD' \`\" != '' ]; then S_CMD_GNU_SED=\"gsed\" ; fi ; wait ; mmmv_polish_ABC_2_A_C_B_exec_t1 'cat ' \" | \$S_CMD_GNU_SED -e 's/[[:blank:]]\+/ /g' | par 70 | \$S_CMD_GNU_SED -e  's/^/    /g'\" " 

alias mmmv_clipboard_text_2_citation_WSL_et_44_t1="S_FP_TMP=\"/tmp/tmp_\`uuid | sed -e 's/-//g'\`.txt\" ; wait ; echo \"--------tsitaadi----algus-------------- \`nice -n 2 powershell.exe -c Get-Clipboard\` --------tsitaadi----lõpp--------------\" | par 40 | sed -e 's/^/    /g' >> \$S_FP_TMP ; wait ; cat \$S_FP_TMP | clip.exe  ; wait ; rm -f \$S_FP_TMP ;"

alias mmmv_clipboard_text_2_citation_WSL_uk_44_t1="S_FP_TMP=\"/tmp/tmp_\`uuid | sed -e 's/-//g'\`.txt\" ; wait ; echo \"--------citation----start-------------- \`nice -n 2 powershell.exe -c Get-Clipboard\` --------citation----end---------------\" | par 40 | sed -e 's/^/    /g' >> \$S_FP_TMP ; wait ; cat \$S_FP_TMP | clip.exe  ; wait ; rm -f \$S_FP_TMP ;"
...
#     cat \$S_FP_TMP_0 | par 50 | \$S_CMD_GNU_SED -e '\${/^[[:blank:]]*\$/d}' | \$S_CMD_GNU_SED -e 's/^/    /g' >> \$S_FP_TMP_1 ; wait ; 
